...
def f_0(t,T2,A,B):
    return A+B*np.exp(-t/T2)


os.chdir(r"C:\Users\billy\OneDrive\Books\iPad 同步\近代物理实验\脉冲核磁共振")
#data=pd.read_csv(r"szx\20190307\1607_tau1000.csv")
#data=pd.read_csv(r"szx\20190307\1715_tau1000.csv")
data=pd.read_csv(r"T1_2.csv")
...
fit=opt.curve_fit(f_0,data['tau'],data['FID'],[15.7/np.log(2),float(data['FID'].tail(1)),float(data['FID'].loc[0]-data['FID'].tail(1))])
x=np.linspace(0,data['tau'].tail(1),100)

plt.plot(data['tau'],data['FID'],'x')
plt.plot(x,f_0(x,*fit[0]))
plt.xlabel(r"$t$/ms",**x_label_settings)
plt.ylabel("FID",**y_label_settings)
plt.legend(("data","fit"))
plt.savefig(r'sequential_huibo_T1_2')
#plt.show()
